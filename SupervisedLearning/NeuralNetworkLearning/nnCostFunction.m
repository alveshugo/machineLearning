...
actuals = eye(num_labels)(y,:);

cost = 0;
for i=1:m
  cost = cost + sum(actuals(i,:) .* log(a3(i,:)) + (1 - actuals(i,:)) .* log(1 - a3(i,:)));
end
...
%
% cost = sum(actuals .* log(a3) + (1 - actuals) .* log(1 - a3), 2);
% J = - sum(cost) / m;
...
for t = 1:m
  ...
  delta_3 = a_3 - actuals(t,:)';

  delta_2 = (Theta2' * delta_3)(2:end, 1) .* sigmoidGradient(z_2);
